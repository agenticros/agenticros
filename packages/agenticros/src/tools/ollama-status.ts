    ...
    async execute(_toolCallId, params) {
      ...
      try {
        ...
        if (!hasVlm && models.length > 0) {
          text += ` Follow Me is currently configured for "${vlmModel}" (not in list). To use what you have: set followMe.vlmModel to "${suggest}" in the AgenticROS plugin config and restart Follow Me. No need to pull another model.`;
        } else if (!hasVlm) {
