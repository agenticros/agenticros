...
export async function initCommand(opts: InitOptions): Promise<void> {
  ...
  if (hasAnyMcpClient || opts.force) {
    ...
    if (wantMcp) {
      await runStep("Configuring MCP clients", async () => {
        await mcpSetupCommand({ repoRoot: paths.repoRoot, quiet: true });
      });
    }
  } else {
    dim("No MCP clients on PATH — skip. Run `agenticros mcp setup` later.");
  }
...
function writeInitNpmrcInline(installDir: string): void {
  ...
  const contents =     [
      ...
      "# Optional native robotics deps (node-datachannel, rclnodejs) may fail in",
      "# sandboxes; init retries with --no-optional when that happens.",
