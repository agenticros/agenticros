...
# `pnpm deploy --prod` resolves rclnodejs fine in the lockfile (it's an
# optionalDependency of @agenticros/core, which packages/agenticros depends
# on) but reliably fails to materialise the package itself inside $DEPLOY_DIR
# — even though node-datachannel, core's *other* optionalDependency, deploys
# correctly every time. This is a pnpm quirk with optional deps that are only
# reachable through a transitive workspace dependency, not something under
# our control, and it silently breaks the "local" transport (ROS2 transport
# not ready, teleop/camera dead) with no error at deploy time — it only
# surfaces later as "Cannot find module 'rclnodejs'" in the gateway log. This
# took down real-robot teleop on 2026-08-10 (the previous deploy directory
# had rclnodejs from some earlier untracked manual step; a routine redeploy
# silently dropped it). Detect and self-heal: if @agenticros/core's own
# node_modules/rclnodejs isn't resolvable inside the deploy tree, reconstruct
# it from the workspace's already-working copy — same pnpm virtual-store
# layout, generated by hand once and now done here on every deploy.
