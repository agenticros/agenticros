...
Missions are higher-level behaviors exposed to the AI agent via **skills** and **mission tools**. The **Follow Me** mission is **native** to OpenClaw + AgenticROS: it uses the robot's camera (ROS2), a Qwen vision model (Ollama), and cmd_vel (ROS2)—no external apps or separate ROS2 nodes.

## Follow Me (native)

**Mission:** The robot finds a person in the room (using a 2D or RealSense camera) and follows them, staying about 1 m behind. The logic runs **inside the AgenticROS plugin**: a control loop grabs camera frames over ROS2, sends them to Ollama (Qwen VLM) for person detection and position/distance, then publishes Twist commands to cmd_vel.
...
2. **Where OpenClaw runs**: Ollama with a vision model (e.g. `ollama pull qwen2-vl:7b`). Configure the AgenticROS plugin if needed:
...
- No dependency on openclaw-robotics or the agenticros_follow_me ROS2 package; everything is in the plugin + Ollama + ROS2 transport.
...
- Add a skill under `packages/agenticros/skills/missions/<name>/SKILL.md`.
