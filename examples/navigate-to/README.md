...
Then `run_mission` with capability `navigate_to` and inputs `{ "x", "y", "yaw?" }`.

See the skill README for Nav2 bringup requirements. This directory previously held the seed manifest; it is now a pointer only.
